...
export function useModelForm({ initialModel, onSubmitSuccess }: UseModelFormProps = {}) {
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      if (model?.id) {
        const response = await api.post(`/models/${model.id}/fetch-models`, {
  ...
  const handleFormSubmit = async (data: ModelFormValues) => {
    ...
    try {
      if (model?.id) {
        ...
        await aiModelService.updateModel(model.id, data as AIModelData);
        toast({
          title: "Model Updated",
        ...
        // Create new model
        await aiModelService.createModel(data as AIModelData);
        toast({
          title: "Model Created",
      ...
      toast({
        ...
        description: "There was an error saving the model. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
